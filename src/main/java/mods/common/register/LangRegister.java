...
{
    private static final LangRegister INSTANCE = new LangRegister();
    ...
    public static LangRegister instance()
    {
        return INSTANCE;
    }
    ...
    private void searchZipForLanguages(File source, Side side) throws IOException
    {
        ...
        for (ZipEntry ze : Collections.list(zf.entries()))
        {
            ...
            if (matcher.matches())
            {
                ...
                LangRegister.instance().injectLanguage(lang, StringTranslate.parseLangFile(zf.getInputStream(ze)));
    ...
    private void searchDirForLanguages(File source, String path, Side side) throws IOException
    {
        for (File file : source.listFiles())
        {
            ...
            {
                String lang = matcher.group(2);
                FMLLog.log(Level.INFO, "Injecting found translation assets for lang %s at %s into language system", lang, currPath);
                LangRegister.instance().injectLanguage(lang, StringTranslate.parseLangFile(new FileInputStream(file)));
                // Ensure en_US is available to StringTranslate on the server
                if ("en_US".equals(lang) && side == Side.SERVER)
                {
